...
def cofiCostFunc(params, Y, R, num_users, num_movies,
    ...
    J=0.5 * np.sum( (R * ( np.matmul(X,Theta.transpose()) - Y ))**2 )   +   lambda_/2 * np.sum(X**2)   +   lambda_/2 * np.sum(Theta**2)

    X_grad=np.matmul(   ( R * ( np.matmul(X,Theta.transpose()) - Y ))    ,   Theta ) + lambda_ * X

    Theta_grad=np.matmul(  (R * ( np.matmul(X,Theta.transpose()) - Y )).transpose()   ,   X) + lambda_* Theta
